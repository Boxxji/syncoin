...
class SynCoinDesktopApp:
    ...
    def setup_ui(self):
        ...
        tk.Label(c1, text="SOLDE RÉMUNÉRATION", bg="#161b22", fg="#8b949e", font=("Segoe UI", 9, "bold")).pack(anchor="w")
        self.lbl_olona = tk.Label(c1, text="100.00 🌱", bg="#161b22", fg="#3fb950", font=("Segoe UI", 18, "bold"))
        self.lbl_olona.pack(anchor="w", pady=(4, 0))
        tk.Label(c1, text="≈ 50.00 € (Convertible SOL/EUR)", bg="#161b22", fg="#8b949e", font=("Segoe UI", 8)).pack(anchor="w")

        # Card 2: Jobs Inférence
        c2 = tk.Frame(cards_frame, bg="#161b22", padx=15, pady=12, relief="groove", bd=1)
        c2.pack(side="left", fill="both", expand=True, padx=6)
        tk.Label(c2, text="TÂCHES IA COMPLÉTÉES", bg="#161b22", fg="#8b949e", font=("Segoe UI", 9, "bold")).pack(anchor="w")
        self.lbl_jobs = tk.Label(c2, text="0", bg="#161b22", fg="#58a6ff", font=("Segoe UI", 18, "bold"))
        self.lbl_jobs.pack(anchor="w", pady=(4, 0))
        tk.Label(c2, text="SLMs / WASM / Bioacoustique", bg="#161b22", fg="#8b949e", font=("Segoe UI", 8)).pack(anchor="w")

        # Card 3: Arbres Plantés ASBL
        c3 = tk.Frame(cards_frame, bg="#161b22", padx=15, pady=12, relief="groove", bd=1)
        c3.pack(side="left", fill="both", expand=True, padx=(6, 0))
        tk.Label(c3, text="IMPACT ÉCOLOGIQUE", bg="#161b22", fg="#8b949e", font=("Segoe UI", 9, "bold")).pack(anchor="w")
        self.lbl_trees = tk.Label(c3, text="0 🌲", bg="#161b22", fg="#2ea043", font=("Segoe UI", 18, "bold"))
        self.lbl_trees.pack(anchor="w", pady=(4, 0))
        tk.Label(c3, text="Arbres plantés via ASBL", bg="#161b22", fg="#8b949e", font=("Segoe UI", 8)).pack(anchor="w")

        # 3. Energy & Solar Gauges
        energy_frame = tk.Frame(self.root, bg="#161b22", padx=15, pady=12)
        energy_frame.pack(fill="x", padx=15, pady=10)

        tk.Label(energy_frame, text="⚡ ARBITRAGE ÉNERGÉTIQUE SOUVERAIN (MICRO-NEOCLOUD)", bg="#161b22", fg="#e3b341", font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(0, 8))
        ...
        self.lbl_solar_val = tk.Label(s_box, text="☀️ Solaire : 1250 W", bg="#161b22", fg="#e3b341", font=("Segoe UI", 11, "bold"))
        ...
        self.lbl_battery_val = tk.Label(b_box, text="🔋 Batterie BESS : 88%", bg="#161b22", fg="#3fb950", font=("Segoe UI", 11, "bold"))
        ...
        self.btn_toggle = tk.Button(
            ctrl_frame,
            text="🚀 DÉMARRER LE CALCUL DÉCARBONÉ (GÉNÉRER DES OLONA)",
            bg="#238636",
            fg="white",
        ...
        tk.Label(log_frame, text="📜 JOURNAL D'INFÉRENCE & PREUVES EN DIRECT", bg="#161b22", fg="#8b949e", font=("Segoe UI", 9, "bold")).pack(anchor="w", pady=(0, 4))
        ...
        self.log_message(f"Device: {platform.system()} {platform.machine()} | Architecture: Metal / CUDA / WASM Ready.")
    ...
    def toggle_mining(self):
        if not self.is_mining:
            self.is_mining = True
            self.btn_toggle.configure(text="🛑 PAUSE DU CALCUL DÉCARBONÉ", bg="#da3633")
            self.status_badge.configure(text="● INFÉRENCE ACTIVE (100% SOLAIRE)", bg="#238636", fg="white")
            self.log_message("Worker thread started. Listening for OpenAI-compatible gateway jobs...")
            self.start_worker_async()
        else:
            self.is_mining = False
            self.btn_toggle.configure(text="🚀 DÉMARRER LE CALCUL DÉCARBONÉ (GÉNÉRER DES OLONA)", bg="#238636")
            self.status_badge.configure(text="● EN ATTENTE", bg="#21262d", fg="#8b949e")
        ...
        def _run():
            ...
            async def _worker_loop():
                while self.is_mining:
                    # Simule la complétion de jobs périodiques en mode live
    ...
    def update_stats_ui(self):
        ...
        trees = int((self.olona_balance - 100.0) / 10.0)
        self.lbl_trees.configure(text=f"{max(0, trees)} 🌲")
        self.log_message(f"Job completed: Inference proof SHA verified. +0.05 Olona credited.")

    def start_telemetry_timer(self):
        def _tick():
            # Variation réaliste des jauges solaires
            if self.is_mining:
                self.solar_watts = max(800.0, min(3200.0, self.solar_watts + (time.time() % 7 - 3) * 20))
                self.lbl_solar_val.configure(text=f"☀️ Solaire : {int(self.solar_watts)} W")
                self.prog_solar["value"] = min(100, int((self.solar_watts / 3500) * 100))
